...
export const meta = {
  title: 'Compositon API と direct-vuex を使って TypeScript に対応する',
  tags: ['Vue.js', 'TypeScript', 'Composition API', 'Vuex'],
  publishDate: '2020-06-18',
  modifiedDate: false,
  seoDescription: 'Vue3 で実装予定の Composition API を使って Vue2 を TypeScript 対応する話',
}
...
コンポーネントとロジックの分離、TypeScriptサポートを念頭に置かれた Vue3 で予定されている新しいコンポーネント設計です。
公式から Vue2 向けの Composition API が公開されていて Vue3 を待たなくても使えます。

公式ドキュメントが分かりやすくまとまっているので詳細を知りたいかたはそちらをご覧ください。
2020/06/18 時点では英語と中国語しかありませんが翻訳でも十分理解できると思います。

https://composition-api.vuejs.org/

まずは npm もしくは yarn から @vue/composition-api をインストールしてください。
...
リアクティブな値を扱うには reactive や ref を使う必要があります。
ここで言うリアクティブな値とは data に書いていた state のことで v-model や methods などで更新されリアクティブに View に反映される値のことです。
setup() の戻り値に関数を返すと methods と同様に template 内で @click などのイベントに使えます。
