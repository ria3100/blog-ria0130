// https://github.com/PKief/vscode-material-icon-theme/blob/master/src/icons/fileIcons.ts
...
export const fetchFileIcon = (filename: string) => {
  const name = filename.split('.')[0]
  const extension = filename.replace(`${name}.`, '')
  ...
  type FileType = { name: string; match: string }

  const matchIcon = fileIcons.icons.reduce<FileType>(
    (fileType, item) => {
      if (fileType.match === 'name') return fileType

      const { fileNames, fileExtensions } = item

      if (fileExtensions?.includes(currentFile.extension)) {
        fileType.name = item.name
        fileType.match = 'extension'
      }

      if (fileNames?.includes(currentFile.filename)) {
        fileType.name = item.name
        fileType.match = 'name'
      }
  ...
  return matchIcon.name || fileIcons.defaultIcon
}
